...
def client_sends():
  ...
  # correct length.
  segment = segments[0]
  print(segment.length, CTCP_HEADER_LEN + len(test_str))
  return (
...
def correct_header_fields():
  ...
  print(segment.seqno == ref_segment.seqno,segment.ackno == ref_segment.ackno,segment.length == ref_segment.length,segment.window == ref_segment.window,segment.checksum == ref_segment.checksum)
  return (
